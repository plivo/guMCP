...
def validate_config(config: Dict[str, Any], server_path: str) -> bool:
    ...
    if missing_fields:
        logger.warning(f"Server {server_path} is missing required fields: {', '.join(missing_fields)}")
        return False
    return True
...
def main():
    parser = argparse.ArgumentParser(description='Generate a JSON file containing server configurations')
    parser.add_argument(
        '--output',
        '-o',
