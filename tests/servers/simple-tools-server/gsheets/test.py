...
async def test_get_sheet_names(client):
    ...
    response = await client.call_tool("get-sheet-names", {"spreadsheet_url": EXAMPLE_URL})
...
async def test_batch_get(client):
    """Test batch-get tool"""
    response = await client.call_tool("batch-get", {
...
async def test_batch_update(client):
    ...
    response = await client.call_tool("batch-update", {
        "spreadsheet_url": EXAMPLE_URL,
...
async def test_append_values(client):
    ...
    response = await client.call_tool("append-values", {
        "spreadsheet_url": EXAMPLE_URL,
        "range": "Sheet1!A1",
        "values": [["New Row", 123]]
    })
...
async def test_lookup_row(client):
    ...
    response = await client.call_tool("lookup-row", {
        "spreadsheet_url": EXAMPLE_URL,
        "range": "Sheet1!A1:C10",
        "value": "New Row"
    })
...
async def test_clear_values(client):
    ...
    response = await client.call_tool("clear-values", {
        "spreadsheet_url": EXAMPLE_URL,
        "range": "Sheet1!A1:C1"
    })
...
async def test_copy_sheet(client):
    ...
    response = await client.call_tool("copy-sheet", {
